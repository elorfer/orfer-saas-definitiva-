                ...
                <form onSubmit={handleSubmit} className="glass-morphism rounded-3xl p-8 md:p-12 relative overflow-hidden">
                    <AnimatePresence mode="wait">
                        ...
                        {step === 3 && (
                            ...
                            >
                                ...
                                <div className="flex lg:grid lg:grid-cols-4 gap-4 lg:gap-6 overflow-x-auto lg:overflow-visible pb-8 lg:pb-0 px-1 snap-x snap-mandatory custom-scrollbar-hide">
                                    {[
                                        { 
                                            id: 'Test Mode', 
                                            price: 1, 
                    ...
                    <div className="flex gap-4 mt-10">
                        ...
                        <button 
                            type="submit"
                            disabled={isLoading}
                            className={`btn-primary flex-1 ${step === 4 ? 'bg-coffee-medium !py-5 flex items-center justify-center gap-3' : ''}`}
                        >
                            {isLoading ? '...' : (
                                step === 4 ? (
                                    <>
                                        <svg className="w-5 h-5 text-black" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/></svg>
                                        {lang === 'es' ? `Finalizar pedido y Pagar ($${formData.price} USD)` : `Finish order and Pay ($${formData.price} USD)`}
                                    </>
                                ) : (
