...
export class RecommendationService {
  ...
  async getRecommendedSong(
    ...
    try {
      ...
      // 🚨 NOTA: El cache NO incluye excludeIds en la clave porque pueden variar
      // En su lugar, verificamos si el resultado del cache está en excludeIds y lo filtramos
      const cacheKey = this.generateCacheKey(currentSongId, genres, userId, offset);
      ...
      if (filteredCandidates.length === 0) {
        ...
        // 🚀 FALLBACK INTELIGENTE: Intentar múltiples estrategias progresivas
        let fallback = null;
        
        // Estrategia 1: Buscar por género amplio (sin filtros estrictos)
        if (!shouldChangeGenre && songGenres.length > 0) {
          this.logger.log(`🔄 [FALLBACK 1] Buscando por género amplio (ignorando artista/afinidad)...`);
          fallback = await this.getSongsByGenreAndPopularity(songGenres, currentSongId, userId, excludeIds);
        }
        
        // Estrategia 1b: Si cambiamos género, buscar otros géneros
        if (shouldChangeGenre && songGenres.length > 0) {
  ...
  private async getSameGenreSongs(currentSong: Song, genres?: string[], excludeIds: string[] = []): Promise<Song[]> {
    ...
    if (songs.length < 10 && targetGenres.length > 0) {
      ...
      // Buscar géneros relacionados (géneros que compartan palabras clave)
      const relatedConditions = targetGenres.map((_, index) => 
        `LOWER(song.genres) LIKE :relatedGenre${index}`
      ).join(' OR ');
   ...
   * ⚡ GESTIÓN DE CACHE
   */
  private generateCacheKey(currentSongId: string, genres?: string[], userId?: string, offset?: number): string {
    ...
    return `rec:${currentSongId}:${genresStr}:${userStr}:${timeWindow}${offsetStr}`;
  }
  ...
  async generatePlaylistBatch(
    ...
    const startTime = Date.now();
    this.logger.log(`🚀 [BATCH] Generando ${count} recomendaciones para semilla: ${seedSongId}`);
  ...
  private getAdaptiveExcludeLimit(totalSongs: number, requestedExcludes: number): number {
    ...
    // Si hay pocas canciones, reducir agresivamente las exclusiones
    if (totalSongs <= 20) {
      // Para catálogos muy pequeños (≤20), excluir máximo 5 IDs
      // Esto permite re-comendación circular
      return Math.min(5, requestedExcludes);
    }
    
    // Para catálogos medianos (21-50), excluir máximo 10 IDs
    return Math.min(10, requestedExcludes);
  }
